...
import caesarC as caesar


class Chat:
    def __init__(self, host='192.0.0.0', port=5555):
        self._socket = socket()
        try:
            self._sokcet.connect((host, port))
...
@chat.messageHandler()
def printer(msg):
    print(f"\033[s\n\033[1A\033[1L{caesar.decode(msg)}\033[1B", end="")
...
def getUserName():
    ...
    colors = [
        ...
        for colour in zip(
            ("Balck", "Red", "Green", "Yellow", "Blue", "Purple", "Cyan", "White"),
            range(30, 38)
        )
    ]
...
chat.setUserName(caesar.encode(username))
...
while True:
    ...
    if func := commandProcessor(msg):
        func()
    try:
        chat.sendMessage(caesar.encode(msg))
    except:
        print("Connection Refused")
